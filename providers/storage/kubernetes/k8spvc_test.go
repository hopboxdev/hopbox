...
func TestEnsureHomeCreatesPVCThenIsIdempotent(t *testing.T) {
	...
	p, err := New(cli, "hopbox-workspaces", "fast", "2Gi")
	if err != nil {
		t.Fatal(err)
	}
	...
	if m1.Source != "hopbox-home-w1" || m1.Target != "/home/dev" {
		t.Fatalf("bad mount: %+v", m1)
	}
	// the PVC actually exists with the requested class + size
	pvc, err := cli.CoreV1().PersistentVolumeClaims("hopbox-workspaces").Get(context.Background(), "hopbox-home-w1", metav1.GetOptions{})
	if err != nil {
...
func TestDeleteRemovesPVCAndIsIdempotent(t *testing.T) {
	cli := fake.NewSimpleClientset()
	p, _ := New(cli, "hopbox-workspaces", "", "1Gi")
	m, _ := p.EnsureHome(context.Background(), ports.HomeRequest{WorkspaceID: "w1"})
	if err := p.Delete(context.Background(), m.Source); err != nil {
		t.Fatal(err)
	}
	if _, err := cli.CoreV1().PersistentVolumeClaims("hopbox-workspaces").Get(context.Background(), m.Source, metav1.GetOptions{}); !apierrors.IsNotFound(err) {
