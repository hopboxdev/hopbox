// Command mesa is the Mesa user CLI: create/ls/rm/shell against mesad.
package main
...
func dial() (mesav1.WorkspaceServiceClient, func(), error) {
	conn, err := grpc.NewClient(apiAddr, grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		return nil, nil, err
	}
	return mesav1.NewWorkspaceServiceClient(conn), func() { _ = conn.Close() }, nil
}
...
func newListCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			defer closer()
			resp, err := client.ListWorkspaces(context.Background(), &mesav1.ListWorkspacesRequest{})
			if err != nil {
				return err
			}
...
func newRmCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			defer closer()
			if _, err := client.DeleteWorkspace(context.Background(), &mesav1.DeleteWorkspaceRequest{NameOrId: args[0]}); err != nil {
				return err
			}
