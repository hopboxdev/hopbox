...
type fakeStream struct {
	recvQueue []*hopboxv1.ShellServerMsg
	sent      [][]byte
}

func (f *fakeStream) Send(m *hopboxv1.ShellClientMsg) error {
	if d := m.GetData(); d != nil {
		f.sent = append(f.sent, append([]byte(nil), d...))
	}
	return nil
}
func (f *fakeStream) Recv() (*hopboxv1.ShellServerMsg, error) {
...
func TestPumpWritesServerDataToStdout(t *testing.T) {
	fs := &fakeStream{recvQueue: []*hopboxv1.ShellServerMsg{
		{Msg: &hopboxv1.ShellServerMsg_Data{Data: []byte("hello")}},
	}}
...
func TestPumpSendsStdinToServer(t *testing.T) {
	fs := &fakeStream{recvQueue: []*hopboxv1.ShellServerMsg{
		{Msg: &hopboxv1.ShellServerMsg_ExitCode{ExitCode: 7}},
	}}
	var out bytes.Buffer
